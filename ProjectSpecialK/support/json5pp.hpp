...
public:
   ...
   */
  value(null_type null) noexcept : type(TYPE_NULL) { null; }
...
private:
  ...
  void parse_number(value& v, int ch)
  {
    ...
    for (;;) {
      if (ch == '0') {
        ...
        if (has_flag(flags::hexadecimal) && ((ch == 'x') || (ch == 'X'))) {
          ...
          v = negative ? (float)(-(long long)int_part) : (float)int_part;
          return;
        }
        break;
      } else if (is_digit(ch)) {
    ...
    if ((frac_part == 0) && (exp_part == 0)) {
      if (negative) {
        const auto integer_value = static_cast<value::integer_type>(-(signed)int_part);
        if (static_cast<decltype(int_part)>(integer_value) == -(signed)int_part) {
          v = integer_value;
          return;
        }
      } else {
    ...
    float number_value = (float)int_part;
    if (frac_part > 0) {
      number_value += (static_cast<float>(frac_part) * std::powf(10.0f, (float)-frac_divs));
    }
    if (exp_part > 0) {
      number_value *= std::powf(10.0f, (float)(exp_negative ? -exp_part : +exp_part));
    }
